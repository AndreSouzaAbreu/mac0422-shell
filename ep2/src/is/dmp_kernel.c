...
PUBLIC void process_queue_dmp()
{
  ...
  /* Now show scheduling queues. */
  printf("Scheduling queues processes.\n");
  for (r=0;r<NR_SCHED_QUEUES; r++) {
      rp = rdy_head[r];
      if (!rp) continue;
      printf("%2d: ", r);
      while (rp != NIL_PROC) {
	  printf("[%2d, %3d, %4d, %4d, %08x] ",
                  rp->p_priority, proc_nr(rp), rp->p_user_time, rp->p_sys_time, rp->p_reg.sp);
          rp = rp->p_nextready;
      }
      printf("\n");
  }
