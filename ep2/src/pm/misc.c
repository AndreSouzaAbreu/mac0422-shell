 ...
 *   do_svrctl: process manager control
     ################ NEW
 *   do_lockpriority: lock the priority of a given process to a given value
 *   do_unlockpriority: unlock the priority of a given process
     ################
 */
 ...
 *				do_lockpriority				     *
 *===========================================================================*/
PUBLIC int do_lockpriority(void)
{
  pid_t cpid;
  int priority, proc_number;
  register struct mproc *childproc;


  cpid = m_in.m1_i1; priority = m_in.m1_i2;
  for (childproc = &mproc[0]; childproc < &mproc[NR_PROCS] && childproc->mp_pid != cpid; childproc++) ;
  if (childproc == &mproc[NR_PROCS])
          return -2;
  if (!(childproc->mp_flags & IN_USE) || childproc->mp_parent != who_p)
          return -2;
  proc_number = childproc->mp_endpoint;
  printf("PM: lockpriority (%d) from process '%s' (%d) to child process '%s' (%d, %d).\n",
                  priority, mp->mp_name, who_p, childproc->mp_name, cpid, proc_number);
  return(sys_lockpriority(proc_number, priority));
}

/*===========================================================================*
 *				do_unlockpriority			     *
 *===========================================================================*/
PUBLIC int do_unlockpriority(void)
{
  pid_t cpid;
  int priority, proc_number;
  register struct mproc *childproc;
  ...
  if (!(childproc->mp_flags & IN_USE) || childproc->mp_parent != who_p)
          return -1;
  proc_number = childproc->mp_endpoint;
  printf("PM: unlockpriority from process '%s' (%d) to child process '%s' (%d, %d).\n",
                  mp->mp_name, who_p, childproc->mp_name, cpid, proc_number);
  return(sys_unlockpriority(proc_number));
}
/* ################################################### */
